...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    ...
    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { full_name: name, theme } }
    });
    if (error) {
      ...
      <div className="auth-page">
              ...
              <div className="auth-mockup__body">
                <div className="auth-mockup__left">
                  ...
                  <div className="auth-mockup__tabs">
                    {["Branding","Tarifs","Domaine"].map(t => (<span key={t} className="auth-mockup__tab">{t}</span>))}
                  </div>
                </div>
                <div className="auth-mockup__right">
                  <div className="auth-mockup__stats">
                    <div className="auth-mockup__stat"><div className="auth-mockup__stat-label">ACCÈS</div><div className="auth-mockup__stat-value">Prêt</div></div>
                    <div className="auth-mockup__stat"><div className="auth-mockup__stat-label">REVENTE</div><div className="auth-mockup__stat-value">Prêt</div></div>
                  </div>
        ...
        <div className="auth-right">
          <div className="auth-form-wrap">
            ...
            <div className="auth-bonus">✦ Vérifiez votre email pour débloquer 3 images gratuites et 1 voix off gratuite.</div>
            ...
            {success && <div className="auth-success">✅ Compte créé ! Vérifiez votre email pour confirmer.</div>}
            ...
            <div className="auth-form">
              ...
              <div className="auth-field">
                ...
                <div className="auth-theme">
                  <button className={`auth-theme__btn ${theme === "light" ? "auth-theme__btn--active" : ""}`} onClick={() => setTheme("light")}>☀ Clair</button>
                  <button className={`auth-theme__btn ${theme === "dark" ? "auth-theme__btn--active" : ""}`} onClick={() => setTheme("dark")}>☾ Sombre</button>
                </div>
              </div>
              <div className="auth-field">
              ...
              <div className="auth-field">
                ...
                <div style={{position:"relative"}}>
                  <input className="auth-input" type={showPass ? "text" : "password"} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} style={{paddingRight:40}} />
              ...
              <button className="auth-btn auth-btn--primary" onClick={handleRegister} disabled={loading}>
              ...
              <p className="auth-switch">Vous avez déjà un compte ? <Link href="/login" className="auth-link">Se connecter</Link></p>
            </div>
          </div>
        </div>
      </div>
...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .auth-mockup__grid-item { background: #111120; border-radius: 6px; padding: 10px; display: flex; flex-direction: column; gap: 4px; }
  ...
  .auth-success { background: rgba(16,185,129,.1); border: 1px solid rgba(16,185,129,.3); border-radius: 8px; padding: 10px 14px; font-size: 13px; color: #10b981; margin-bottom: 16px; }
